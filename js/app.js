(function () {
  ...
  const routeHref = (cat, id) => hrefFor(`/${cat}/${id}`);
  const categoryHref = (id) => hrefFor(`/${id}`);
  const normalizeBodyLinks = (html) => String(html || '').replace(/href="(\/[^"#?.]*[^"#?\/])"/g, (m, href) => `href="${hrefFor(href)}"`);
  ...
  function sectionsHTML(sections) {
    return sections.map((s, i) => `<section class="colony-section"><h3><span>${String(i + 1).padStart(2, '0')}</span>${esc(s.h)}</h3>${normalizeBodyLinks(s.body)}${s.list ? `<div class="task-grid">${s.list.map((x, n) => `<div><b>${priority[(n + i) % priority.length]}</b>${esc(x)}</div>`).join('')}</div>` : ''}</section>`).join('');
  }
  function relatedBlock(p) {
    return `<div class="related-orders">${relatedPages(p).map((r) => `<a href="${esc(hrefFor(r.href))}">${esc(r.label)}</a>`).join('')}</div>`;
  }
  function relatedPages(p, count = 5) {
    const sameCategory = D.pages
      ...
      .slice(0, count)
      .map((candidate) => ({ label: candidate.title, href: routeHref(candidate.category, candidate.id) }));
  ...
  function runSearch(q) {
    ...
    searchResults.innerHTML = matches.length ? matches.map((m) => `<a href="${esc(hrefFor(m.href))}">${esc(m.title)}<span>${esc(m.sub)}</span></a>`).join('') : '<div class="empty">No colony records match.</div>';
