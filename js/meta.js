(function () {
  ...
  function seoFor(route) {
    ...
    let title = S.shortName + ' - Colony Survival Archive';
    let description = S.defaultDescription;
    let keywords = ['RimWorld wiki', 'RimWorld guide'];
    let ogType = 'website';
    if (info.kind === 'category') {
      title = info.category.title + ' | ' + S.titleSuffix;
      description = clip(info.category.summary + ' Practical RimWorld guides with strategy tips, mistakes, FAQ and related links.');
      keywords = ['RimWorld ' + info.category.title.toLowerCase(), 'RimWorld wiki', 'RimWorld guide'];
    } else if (info.kind === 'detail') {
      title = info.page.title + ' | ' + S.titleSuffix;
      description = clip(info.page.summary);
      keywords = ['RimWorld ' + info.page.title.toLowerCase(), 'RimWorld ' + info.category.title.toLowerCase(), 'RimWorld wiki'];
      ogType = 'article';
    } else if (info.kind === 'info') {
      title = info.page.title + ' | ' + S.titleSuffix;
      description = clip(info.page.body);
    } else if (info.kind === 'unknown') {
      title = 'Colony Record Missing | ' + S.titleSuffix;
    }
